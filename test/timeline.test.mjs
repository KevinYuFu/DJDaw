...
for (const pxPerSec of [16, 32, 64, 128, 256]) {
  const gap = gridBeats(120, pxPerSec) * secPerBeat(120) * pxPerSec
  ok(`at ${pxPerSec}px a second the lines sit ${Math.round(gap)}px apart`, gap >= 80 && gap <= 320)
}
...
eq('a dropped track lands on a bar whatever is being drawn', snapToBar(3.1, 120), 4)
