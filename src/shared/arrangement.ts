...
/** Shortest a clip can be dragged, in seconds. */
...
export interface ArrangementClip {
  ...
  startSec: number
  durationSec: number
  /** Where inside the file it starts playing. */
...
/** The clip before a given one, ignoring the clip itself. */
...
/** The clip after a given one, ignoring the clip itself. */
...
export function startEdgeRange(
  ...
  const room = Math.min(clip.sourceOffsetSec, clip.startSec)
...
export function endEdgeRange(
  ...
  const left = clip.sourceDurationSec - clip.sourceOffsetSec
  const latest = Math.min(clip.startSec + left, after ? after.startSec : Number.POSITIVE_INFINITY)
  ...
  return clips.map((c) =>
    c.id === id
      ? {
          ...
          sourceOffsetSec: c.sourceOffsetSec + shift
        }
      : c
  )
}
 ...
 *
 * A track at either end stays where it is rather than wrapping around, so
 * holding the key down does not send it back to the other side.
 */
export function moveTrack(ids: readonly string[], id: string, by: number): string[] {
...
/** One region for the engine: a stretch of timeline read from one file. */
...
/**
 * What one file plays on one track, as regions on the arrangement timeline.
 *
...
export function voiceRegions(
  ...
  if (timelineSec > from) {
    ...
    out.push({ startSec: from, durationSec: timelineSec - from, sourceOffsetSec: past + 1 })
  }
  return out
}
