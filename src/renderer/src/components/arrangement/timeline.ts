 ...
 *
 * The arrangement has a tempo of its own, so the grid is musical rather than
 * a count of seconds: lines fall on bars and their divisions, and how fine a
 * division is drawn follows the zoom. Everything that lands on the timeline
 * snaps to it.
 */
...
export function gridBeats(bpm: number, pxPerSec: number): number {
  ...
  if (!(perBeat > 0)) return DIVISIONS[0]
