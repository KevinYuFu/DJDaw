...
/** The arrangement's ruler and its top bar. */
const ARRANGE_CHROME_H = 62
/** One arrangement lane: shorter than an editing row, so six of them fit. */
const ARRANGE_LANE_MIN_H = 56
const ARRANGE_LANE_H = 74

/**
 * Each view wants a different amount of height and gets its own, so the
...
const VIEW_HEIGHTS: Record<ViewName, { min: number; preferred: number }> = {
  ...
  arrangement: {
    min: ARRANGE_CHROME_H + 2 * ARRANGE_LANE_MIN_H,
...
export function App(): ReactElement {
  ...
  return (
    <div className={classes.join(' ')} style={layout}>
      ...
      {view === 'arrangement' ? (
        <main className="arrangement-area">
          <ArrangementView />
        </main>
      ) : view === 'edit' ? (
